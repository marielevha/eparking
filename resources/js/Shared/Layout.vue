...
<!--          <div class="md:text-md flex items-center justify-between p-4 w-full text-sm bg-white border-b md:px-12 md:py-0">
          ...
          <div class="md:text-md flex items-center justify-start p-4 w-full text-sm bg-white border-b md:px-12 md:py-0">
            <div class="mr-auto mt-1">{{ auth.user.account.name }}</div>
            <dropdown class="mt-1" placement="bottom-end">
              <template #default>
                <div class="group flex items-center cursor-pointer select-none">
                  <div class="mr-1 text-gray-700 group-hover:text-indigo-600 focus:text-indigo-600 whitespace-nowrap">
              ...
              <template #dropdown>
                <div class="mt-2 py-2 text-sm bg-white rounded shadow-xl">
                  <Link class="block px-6 py-2 hover:text-white hover:bg-indigo-500" :href="`/users/${auth.user.id}/edit`">{{ $t('top_bar.profile') }}</Link>
                  <Link v-if="isPermit('user.read', permissions)" class="block px-6 py-2 hover:text-white hover:bg-indigo-500" href="/users">{{ $t('top_bar.manage_user') }}</Link>
                  <Link class="block px-6 py-2 w-full text-left hover:text-white hover:bg-indigo-500" href="/logout" method="delete" as="button">{{ $t('top_bar.logout') }}</Link>
                </div>
              </template>
            </dropdown>
            <language-selector />
        ...
        <div class="md:flex md:flex-grow md:overflow-hidden">
          ...
          <div class="px-2 py-8 md:flex-1 md:p-4 md:overflow-y-auto" scroll-region>
            <flash-messages />
            <slot />
          </div>
        </div>
...
export default {
  ...
  created() {
    //console.log(this.$attrs['permissions'])
  },
